...
# 检查参数是否已配置
if [[ "$APP_ID" == *"YOUR_APP_ID_HERE"* ]] || [[ "$APP_SECRET" == *"YOUR_APP_SECRET_HERE"* ]]; then
    echo "❌ 请先配置APP_ID和APP_SECRET"
    echo "请编辑此文件，更新为你的飞书应用配置"
    exit 1
fi
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] 飞书推送结果: $SEND_RESPONSE" >> "$LOG_FILE"

# 检查响应
SEND_CODE=$(echo "$SEND_RESPONSE" | jq -r '.code // 99999')
if [ "$SEND_CODE" == "0" ]; then
    echo "✅ 飞书推送成功！"
    echo "消息已发送到用户: $RECEIVER_ID"
else
