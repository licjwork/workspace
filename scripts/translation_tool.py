...
def translate_text(text, source_lang="en", target_lang="zh", api_key=None):
    """
    使用LibreTranslate API翻译文本
    
    Args:
        text (str): 要翻译的文本
        source_lang (str): 源语言代码，默认'en'
        target_lang (str): 目标语言代码，默认'zh'
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if translated_text:
                ...
                print(f"翻译API返回空结果: {result}")
                return text
        else:
            print(f"翻译API请求失败，状态码: {response.status_code}")
...
def translate_news_text(text):
    ...
    if translated_text == text:
        print("LibreTranslate翻译失败，使用备用翻译")
        translated_text = backup_translation(text)
    
    # 清理格式和标点符号
    import re
    translated_text = re.sub(r'\s+', ' ', translated_text).strip()
    
    return f"【LibreTranslate翻译】{translated_text}"
