...
try:
    ...
    from playwright_weibo_publisher_fixed import PlaywrightWeiboPublisher
    ...
    async def run_full_workflow(self, topic, images_count=3, publish=False):
        ...
        try:
            ...
            if not await publisher.check_login_status():
                ...
                if publish:
                    success = await publisher.publish_weibo(content, image_paths)
                    if success:
                        print("✅ 微博发布成功！")
                    else:
                        print("❌ 微博发布失败")
...
async def main():
    parser = argparse.ArgumentParser(description='微博全自动发布工具 (全技能整合版) - 修复版 v2')
