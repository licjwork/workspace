...
try:
    ...
    from stable_publisher import StableWeiboPublisher
...
class UnifiedWeiboSkillOrchestrator:
    ...
    async def run_full_workflow(self, topic, images_count=3, publish=False):
        ...
        try:
            ...
            if publish:
                # 使用稳定的移动端发布器
                publisher = StableWeiboPublisher(topic)
...
async def main():
    parser = argparse.ArgumentParser(description='微博全自动发布工具 (最终版)')
