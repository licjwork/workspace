...
class FlexibleWeiboPublisherV4:
    def __init__(self):
    ...
    def publish_auto_hot_search(self):
        ...
        # 重复检查
        print("\n🔍 正在检查是否重复...")
        is_duplicate, similar_topic = self.duplicate_checker.is_duplicate(default_topic, content)
        if is_duplicate:
        ...
        if result:
            # 记录发布历史
            self.duplicate_checker.record_publish(default_topic, content)
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        publisher = FlexibleWeiboPublisherV4()
