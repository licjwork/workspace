...
class EnhancedWeiboPublisher:
    ...
    def get_hot_topics(self):
        ...
        try:
            # 导航到热搜页面
            self.publisher.connect_browser()
            ...
            script = """
            const topics = [];
            document.querySelectorAll('.td-02 a').forEach((el, index) => {
    ...
    def generate_content(self, topic, is_auto_select=False):
        """生成微博内容"""
        if is_auto_select:
            title = f"🔥 💥 热搜第1！{topic}引发全民热议"
            content = f"""
📰 实时热搜TOP1话题深度解析：
...
"""
    ...
    def publish_auto_hot_topic(self):
        """自动获取热搜第一发布"""
        print("🎯 开始自动获取热搜发布...")
        ...
        # 生成内容
        title, content = self.generate_content(selected_topic, is_auto_select=True)
        print(f"🔥 生成标题: {title}")
    ...
    def publish_custom_topic(self, custom_topic):
        """发布指定话题"""
        print(f"🎯 开始发布指定话题: {custom_topic}")
        
        # 生成内容
        title, content = self.generate_content(custom_topic, is_auto_select=False)
        print(f"🔥 生成标题: {title}")
        print(f"📝 内容长度: {len(content)} 字")
...
def main():
    ...
    enhanced_publisher = EnhancedWeiboPublisher()
    
    if len(sys.argv) > 1:
        # 指定话题模式
        custom_topic = sys.argv[1]
        enhanced_publisher.publish_custom_topic(custom_topic)
    else:
        # 自动热搜模式
        enhanced_publisher.publish_auto_hot_topic()
