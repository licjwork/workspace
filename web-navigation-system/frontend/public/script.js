...
async function loadSites() {
    try {
        ...
        const response = await fetch(`/nav/api/sites?${params}`);
...
async function toggleFavorite(siteId) {
    ...
    try {
        if (site.is_favorite) {
            // 取消收藏
            const response = await fetch(`/nav/api/favorites/${siteId}?${params}`, {
                method: 'DELETE'
            });
            ...
            // 添加收藏
            const response = await fetch('/nav/api/favorites', {
                method: 'POST',
                headers: {
...
async function addSite(siteData) {
    try {
        const response = await fetch('/nav/api/sites', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function loadNews() {
    try {
        const response = await fetch('/nav/api/news');
...
function toggleFavorite(siteId) {
    ...
    if (site.is_favorite) {
        // 取消收藏
        fetch(`/nav/api/favorites/${siteId}?${params}`, {
            method: 'DELETE'
        })
        .then(response => {
            if (response.ok) {
